  ...
  <div class="dashboard">
    <div class="container">
      ...
      <div v-else class="table-container">
        <DataTable :data="paginatedData" :columns="tableColumns" :actions="tableActions" result-label="orders"
        ...
        <Pagination v-model="currentPage" :total-items="filteredData.length" :page-size="itemsPerPage"
          :page-size-options="[5, 10, 20, 50]" item-label="orders" :show-first-last="true" :show-jump-to="true"
          @update:page-size="handlePageSizeChange" @page-change="handlePageChange" />
                    ...
                    <tr v-for="(sell, i) in (selectedOrder.sell || [])" :key="i">
                      <td>
                        <div class="sell-product-cell">
                          <img v-if="sell.product_photo" :src="`${baseUrl}/${sell.product_photo}`" class="sell-thumb" />
...
const filteredData = computed(() => {
  let data = items.value.filter(o => {
    const nm = !searchFilters.value.search || o.recipient_name?.toLowerCase().includes(searchFilters.value.search.toLowerCase());
    const st = !searchFilters.value.status || o.status === searchFilters.value.status;
    const dt = !searchFilters.value.date || o.created_at?.startsWith(searchFilters.value.date);
    return nm && st && dt;
  });
...
const loadData = async () => {
  ...
  const params = { page: currentPage.value, limit: itemsPerPage.value };
  ...
  if (sortConfig.value.by) { params.sort_by = sortConfig.value.by; params.sort_order = sortConfig.value.order; }
...
const handlePageSizeChange = (sz) => { itemsPerPage.value = sz; currentPage.value = 1; loadData(); };
