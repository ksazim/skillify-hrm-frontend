        ...
        >
          <template #cell-photos="{ row }">
            <div class="photo-thumbs">
              <img
                v-for="(photo, i) in (row.photos || []).slice(0, 3)"
                :key="i"
                :src="baseUrl+'/' + photo.file_path + '/' + photo.file_name"
                class="thumb"
              />
            ...
            <form @submit.prevent="submitForm" novalidate>
              <div class="form-grid">
                ...
                <div class="form-group full">
                  ...
                  <input v-model="form.product_name" type="text" placeholder="Enter product name..." class="form-input" />
                ...
                <!-- Stock (Yes / No) -->
                <div class="form-group">
                  <label>In Stock <span class="req">*</span></label>
                ...
                <div class="form-group">
                  ...
                  <input v-model="form.regular_price" type="number" step="0.01" min="0" placeholder="0.00" class="form-input" />
                ...
                <div class="form-group">
                  ...
                  <input v-model="form.offer_price" type="number" step="0.01" min="0" placeholder="0.00" class="form-input" />
                </div>

                <!-- Description -->
                <div class="form-group full">
                  <label>Description <span class="req">*</span></label>
                  <textarea v-model="form.description" rows="4" placeholder="Enter product description..." class="form-input"></textarea>
                ...
                <!-- Photo Management -->
                <div class="form-group full">
                  <label>Product Photos <span v-if="modalMode === 'create'" class="req">*</span></label>
          ...
          <div class="modal-body" v-if="selectedItem">
            <div class="view-grid">
              <div class="view-photos">
                <div class="photo-grid">
                  <img
                    v-for="(photo, i) in (selectedItem.photos || [])"
                    :key="i"
                    :src="baseUrl+'/' + photo.file_path + '/' + photo.file_name"
                    class="view-photo"
                  />
              ...
              <div class="view-details">
                <div class="detail-row"><span class="detail-label">Name</span><span>{{ selectedItem.product_name }}</span></div>
                <div class="detail-row"><span class="detail-label">Category</span><span>{{ selectedItem.category?.name ?? '—' }}</span></div>
                ...
                <div class="detail-row"><span class="detail-label">Regular Price</span><span class="price">{{ formatPrice(selectedItem.regular_price) }}</span></div>
                <div class="detail-row"><span class="detail-label">Offer Price</span><span class="offer-price">{{ selectedItem.offer_price ? formatPrice(selectedItem.offer_price) : '—' }}</span></div>
                <div class="detail-row"><span class="detail-label">Views</span><span>{{ selectedItem.view ?? 0 }}</span></div>
                <div class="detail-row"><span class="detail-label">Orders</span><span>{{ selectedItem.orders ?? 0 }}</span></div>
                <div class="detail-row full"><span class="detail-label">Description</span><p class="detail-desc">{{ selectedItem.description }}</p></div>
...
.spinner-large {
  ...
  border: 4px solid #e9ecef; border-top-color: #6366f1;
...
.retry-btn:hover { background: #4f51d8; }

/* ─── Table helpers ─────────────────────────────────────────── */
.table-container { padding: 40px; }
.photo-thumbs { display: flex; gap: 4px; align-items: center; flex-wrap: wrap; }
.thumb {
  width: 34px; height: 34px; object-fit: cover;
  border-radius: 5px; border: 1px solid #dee2e6;
}
.thumb-more { font-size: 12px; color: #6c757d; font-weight: 600; }
.price        { font-weight: 700; color: #1e1e2e; }
.offer-price  { font-weight: 600; color: #e67e22; }
.no-offer, .no-cat { color: #adb5bd; }
.cat-badge {
...
.modal-box {
  ...
  max-height: 90vh; overflow: hidden; width: 100%;
...
.btn-submit {
  padding: 10px 26px; background: #1e1e2e; color: #fff;
  border: none; border-radius: 8px; font-size: 14px; font-weight: 600;
  cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all .2s;
}
.btn-submit:hover:not(:disabled) { background: #2d2d44; box-shadow: 0 4px 12px rgba(0,0,0,.2); }
...
.chip-active {
  background: #1e1e2e !important; border-color: #1e1e2e !important;
  color: #fff !important;
}
.chip-color.chip-active .color-dot { box-shadow: 0 0 0 2px #fff, 0 0 0 3px #1e1e2e; }

.color-dot {
  width: 12px; height: 12px; border-radius: 50%;
  border: 1px solid rgba(0,0,0,.15); display: inline-block; flex-shrink: 0;
}
...
.preview-label { font-size: 12px; color: #6b7280; font-weight: 600; flex-shrink: 0; margin-top: 2px; }
...
.upload-placeholder p { margin: 4px 0; font-size: 14px; color: #374151; }
.upload-hint { font-size: 12px; color: #9ca3af !important; }

.existing-photos { margin-bottom: 16px; }
.section-label { font-size: 12px; font-weight: 700; color: #6b7280; margin: 0 0 10px; text-transform: uppercase; letter-spacing: .5px; }
...
.photo-card {
  ...
  border: 1.5px solid #e5e7eb; aspect-ratio: 1; background: #f9fafb;
}
...
.photo-delete-btn:hover:not(:disabled) { background: #dc3545; transform: scale(1.1); }
.photo-delete-btn:disabled { opacity: .6; cursor: not-allowed; }
.photo-name {
...
/* ─── View modal ────────────────────────────────────────────── */
.view-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
.view-photo { width: 100%; border-radius: 8px; border: 1px solid #e5e7eb; }
.no-photos { color: #9ca3af; font-style: italic; font-size: 14px; }
.view-details { display: flex; flex-direction: column; gap: 14px; }
.detail-row { display: flex; flex-direction: column; gap: 3px; }
.detail-label { font-size: 11px; font-weight: 700; color: #9ca3af; text-transform: uppercase; letter-spacing: .6px; }
.detail-desc { margin: 4px 0 0; color: #374151; line-height: 1.6; font-size: 14px; }
.variants-view { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 4px; }
.variant-chip {
  padding: 4px 12px; background: #f3f4f6; border: 1px solid #e5e7eb;
  border-radius: 20px; font-size: 13px; color: #374151; font-weight: 600;
}
.chip-color-view {
