<template>
  <CrudLayout
    ...
    subtitle="Manage tasks, assignments and deadlines"
    add-label="New Task"
    ...
    error-title="Failed to Load Tasks"
  ...
  >
    ...
    >
      ...
      <template #cell-assigned_to="{ value }">
        <span v-if="value" class="assignee-badge">
          {{ value?.name ?? value }}
        </span>
        <span v-else class="cell-dash">—</span>
      </template>
  ...
  <BaseModal
    ...
    delete-message="This task will be permanently removed from the system."
...
const pageStats = computed(() => [
  ...
  {
    label: "To Do",
    value: items.value.filter((i) => i.status === "todo").length,
  },
  {
    label: "In Progress",
  ...
  {
    label: "Done",
    value: items.value.filter((i) => i.status === "done").length,
  },
]);
...
const formFields = computed(() => [
  {
    ...
    description: "A short, clear title describing the task",
  },
  {
    name: "description",
    label: "Description",
...
const loadData = async () => {
  ...
  const result = await crudStore.fetchAll("/tasks", params);

  if (!result.success) {
    notify("error", result.error?.message || "Failed to load tasks.");
    return;
  }
...
const openEditModal = async (item) => {
  const result = await crudStore.fetchById("/tasks/", item.id);
  if (result.success) {
    modalMode.value = "edit";
    selectedItem.value = { ...result.data };
    showFormModal.value = true;
  } else {
    notify("error", result.error?.message || "Could not load task.");
  }
};
...
const handleFormSubmit = async (data, mode, initialData) => {
  ...
  let result;
  if (mode === "create") {
    result = await crudStore.create("/tasks", fd);
  } else {
    fd.append("_method", "PUT");
    result = await crudStore.post(`/tasks/${initialData.id}`, fd);
  }
...
const handleDeleteSubmit = async (data) => {
  const result = await crudStore.delete("/tasks/", data.id);
  if (!result.success)
...
const handleDeleteSuccess = () => {
  ...
  notify("success", "Task deleted successfully.", { autoClose: 3000 });
  loadData();
};
