<template>
  <div class="attendance-page">
      ...
      <div class="clock-card" :class="clockCardClass">
        ...
        <div class="clock-info">
          ...
          <div class="meta-strip">
            <div class="meta-chip" :class="locationStatus === 'granted' ? 'chip-ok' : locationStatus === 'denied' ? 'chip-err' : 'chip-idle'">
    ...
    <div class="today-summary-bar" v-if="todayLogs.length > 0">
      ...
      <div class="tsb-chips">
        <span
          v-for="log in [...todayLogs].reverse()"
          :key="log.id"
          class="tsb-chip"
          ...
          <div class="modal-panel" :class="isClockedIn ? 'modal-out' : 'modal-in'">
            ...
            <div class="modal-body">
              ...
              <div class="reason-presets">
                <button
                  v-for="preset in (isClockedIn ? clockInPresets : clockOutPresets)"
                  :key="preset"
                  class="preset-chip"
                  :class="{ 'preset-active': reasonModal.reason === preset }"
              ...
              <div class="reason-input-wrap" :class="{ 'input-error': reasonModal.touched && !reasonModal.reason.trim(), 'input-focus': reasonModal.focused }">
                ...
                <textarea
                  ...
                  rows="3"
                  maxlength="300"
                  @focus="reasonModal.focused = true; reasonModal.touched = true"
                  @blur="reasonModal.focused = false"
                  ref="reasonTextarea"
                ></textarea>
              </div>
            ...
            <div class="modal-footer">
              ...
              <button
                ...
                :class="isClockedIn ? 'confirm-out' : 'confirm-in'"
                :disabled="clockLoading || !reasonModal.reason.trim()"
...
const ringColor = computed(() => isClockedIn.value ? '#C9A96E' : 'rgba(201,169,110,0.25)')
...
/* ══════════════════════════════════════════════════
   DESIGN TOKENS
══════════════════════════════════════════════════ */
@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;500;600&family=DM+Mono:wght@300;400;500&family=Outfit:wght@300;400;500;600&display=swap');

:root {
  --gold:          #C9A96E;
  --gold-dim:      rgba(201,169,110,0.20);
  --gold-mid:      rgba(201,169,110,0.55);
  --gold-hover:    rgba(201,169,110,0.30);
  --cream:         #F0EAE0;
  --cream-2:       #BDB5AC;
  --cream-3:       #7A736D;
  --cream-ghost:   rgba(240,234,224,0.10);
  --green:         #6ECFA9;
  --green-dim:     rgba(110,207,169,0.16);
  --green-border:  rgba(110,207,169,0.45);
  --red:           #EF6B6B;
  --red-dim:       rgba(239,107,107,0.16);
  --red-border:    rgba(239,107,107,0.45);
  --blue:          #85B7EB;
  --blue-dim:      rgba(133,183,235,0.14);
  --blue-border:   rgba(133,183,235,0.40);
  --card-bg:       #17171D;
...
.ambient-orb   { position: absolute; border-radius: 50%; filter: blur(90px); opacity: .11; }
.orb-1 {
...
.orb-2 {
  ...
  background: radial-gradient(circle, #6ECFA9 0%, transparent 70%);
...
.page-header, .clock-section, .stats-strip,
.today-summary-bar, .logs-section { position: relative; z-index: 1; }
...
.page-title {
  ...
  font-family: var(--font-serif); color: var(--cream); letter-spacing: .02em;
}
.page-subtitle { margin: 3px 0 0; font-size: 12px; color: var(--cream-2); letter-spacing: .04em; }
...
.clock-label {
  margin-top: 4px; font-size: 10px; color: var(--cream-2);
...
/* ══════════════════════════════════════════════════
   CLOCK CARD
══════════════════════════════════════════════════ */
.clock-section { margin-bottom: 24px; color: white; }
.clock-card {
  color: white;
  background: var(--card-bg); border: 1px solid var(--border);
  padding: 36px 40px; display: flex; align-items: center; gap: 48px; flex-wrap: wrap;
...
.card-in   { border-color: rgba(201,169,110,0.50); }
.card-in::before {
  background: linear-gradient(135deg, rgba(201,169,110,0.07) 0%, transparent 55%);
  opacity: 1;
}

.status-ring-wrap { position: relative; width: 120px; height: 120px; flex-shrink: 0; }
.status-ring      { width: 120px; height: 120px; transform: rotate(-90deg); }
...
.ring-status-dot {
  ...
  background: var(--cream-3); margin-bottom: 4px; transition: background .3s;
}
.card-in .ring-status-dot { background: var(--gold); }
...
.ring-elapsed-label {
  font-size: 9px; text-transform: uppercase; letter-spacing: .10em; color: var(--cream-2);
}

.clock-info { flex: 1; min-width: 220px; display: flex; flex-direction: column; gap: 18px; }
.clock-status-badge {
  display: inline-flex; align-items: center; gap: 8px;
  padding: 5px 14px; border: 1px solid; width: fit-content;
  font-size: 11px; font-weight: 600; letter-spacing: .09em; text-transform: uppercase;
}
.badge-dot { width: 6px; height: 6px; border-radius: 50%; }
.badge-out { color: var(--cream-2); border-color: rgba(189,181,172,0.35); background: rgba(189,181,172,0.07); }
.badge-out .badge-dot { background: var(--cream-2); }
.badge-in  { color: var(--gold); border-color: var(--gold-mid); background: var(--gold-dim); }
.badge-in .badge-dot { background: var(--gold); animation: dotPulse 2s ease-in-out infinite; }

.clock-times { display: flex; align-items: center; gap: 20px; flex-wrap: wrap; }
.time-block  { display: flex; flex-direction: column; gap: 3px; }
.time-block-label { font-size: 10px; text-transform: uppercase; letter-spacing: .08em; color: var(--cream-2); }
.time-block-value { font-family: var(--font-mono); font-size: 18px; color: var(--cream); }
.no-record        { font-size: 14px; color: var(--cream-3); font-family: var(--font-body); }
.time-block-divider { width: 1px; height: 32px; background: var(--border); flex-shrink: 0; }

.meta-strip { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.meta-chip {
...
.chip-ok     { color: var(--green); border-color: var(--green-border); background: var(--green-dim); }
.chip-err    { color: var(--red);   border-color: var(--red-border);   background: var(--red-dim); }
.chip-idle   { color: var(--cream-2); border-color: rgba(189,181,172,0.28); background: rgba(189,181,172,0.06); }
.chip-device { color: var(--blue);  border-color: var(--blue-border);  background: var(--blue-dim); }

.clock-btn {
  display: inline-flex; align-items: center; gap: 10px;
  padding: 13px 30px; border: 1px solid; cursor: pointer;
  font-family: var(--font-body); font-size: 13px; font-weight: 600;
  letter-spacing: .08em; text-transform: uppercase; transition: all .2s; width: fit-content;
}
.clock-btn svg { width: 16px; height: 16px; flex-shrink: 0; }
.btn-in { background: var(--gold-dim); border-color: var(--gold-mid); color: var(--gold); }
.btn-in:hover:not(:disabled) {
  background: var(--gold-hover); border-color: var(--gold);
  box-shadow: 0 0 28px rgba(201,169,110,0.25);
}
.btn-out { background: var(--red-dim); border-color: var(--red-border); color: var(--red); }
.btn-out:hover:not(:disabled) {
  background: rgba(239,107,107,0.26); border-color: var(--red);
  box-shadow: 0 0 28px rgba(239,107,107,0.20);
}
.clock-btn:disabled { opacity: .55; cursor: not-allowed; }

.btn-spinner {
  width: 16px; height: 16px; border-radius: 50%; flex-shrink: 0;
  border: 2px solid rgba(201,169,110,0.30); border-top-color: var(--gold);
  animation: spin .7s linear infinite;
}
@keyframes spin { to { transform: rotate(360deg); } }

.action-message { margin: 0; font-size: 12px; font-weight: 500; letter-spacing: .03em; }
...
.stats-strip {
  ...
  background: var(--card-bg); border: 1px solid var(--border); margin-bottom: 20px;
}
.stat-tile {
  flex: 1 1 0; display: flex; align-items: center; gap: 16px;
  padding: 22px 24px; min-width: 140px;
}
.stat-tile-divider { width: 1px; background: var(--border); align-self: stretch; flex-shrink: 0; }
.stat-tile-icon {
  width: 40px; height: 40px; border: 1px solid; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center;
}
.stat-tile-icon svg { width: 18px; height: 18px; }
.icon-checkins  { border-color: var(--gold-mid);    color: var(--gold);   background: var(--gold-dim); }
.icon-checkouts { border-color: var(--blue-border); color: var(--blue);   background: var(--blue-dim); }
.icon-status    { border-color: var(--green-border);color: var(--green);  background: var(--green-dim); }
.icon-location  { border-color: rgba(189,181,172,0.35); color: var(--cream-2); background: rgba(189,181,172,0.08); }
.stat-tile-body  { display: flex; flex-direction: column; gap: 4px; }
.stat-tile-value { font-family: var(--font-mono); font-size: 18px; font-weight: 500; color: var(--cream); line-height: 1; }
.stat-tile-label { font-size: 10px; text-transform: uppercase; letter-spacing: .08em; color: var(--cream-2); }
...
.tsb-label {
  ...
  color: var(--cream-2); white-space: nowrap; flex-shrink: 0;
}
.tsb-chips { display: flex; gap: 8px; flex-wrap: wrap; }
.tsb-chip {
  display: inline-flex; align-items: center; gap: 5px;
  padding: 3px 10px; border: 1px solid;
  font-family: var(--font-mono); font-size: 11px; font-weight: 500;
}
.tsb-chip svg { width: 9px; height: 9px; flex-shrink: 0; }
.tsb-in  { color: var(--gold); border-color: var(--gold-mid);    background: var(--gold-dim); }
.tsb-out { color: var(--blue); border-color: var(--blue-border); background: var(--blue-dim); }
...
.logs-header-left  { display: flex; align-items: baseline; gap: 12px; }
.logs-title {
  margin: 0; font-family: var(--font-serif); font-size: 18px; font-weight: 500;
  color: var(--cream); letter-spacing: .02em;
}
.logs-count { font-size: 11px; color: var(--cream-2); letter-spacing: .05em; }

.btn-refresh {
  display: inline-flex; align-items: center; gap: 7px;
  padding: 7px 14px; background: transparent;
  border: 1px solid rgba(189,181,172,0.28); color: var(--cream-2);
  font-family: var(--font-body); font-size: 11px; font-weight: 500;
  letter-spacing: .06em; text-transform: uppercase; cursor: pointer; transition: all .14s;
}
.btn-refresh:hover:not(:disabled) { border-color: var(--gold-mid); color: var(--gold); }
.btn-refresh:disabled { opacity: .4; cursor: not-allowed; }
.btn-refresh svg { width: 13px; height: 13px; }
.btn-refresh svg.spinning { animation: spin .7s linear infinite; }

/* ── Skeleton ── */
.logs-skeleton { display: flex; flex-direction: column; gap: 16px; }
...
.sk-icon   { width: 36px; height: 36px; border-radius: 2px; flex-shrink: 0; }
.sk-type   { height: 13px; width: 90px; }
.sk-meta   { height: 10px; width: 160px; }
.sk-reason { height: 10px; width: 220px; }
.sk-time-r { height: 14px; width: 55px; flex-shrink: 0; }

/* ── Timeline ── */
.logs-timeline { display: flex; flex-direction: column; }
.log-entry {
  display: flex; align-items: flex-start; gap: 16px;
  padding: 15px 10px; border-bottom: 1px solid var(--border-faint);
  transition: background .13s; border-radius: 2px;
}
.log-entry:last-child { border-bottom: none; }
.log-entry:hover      { background: rgba(255,255,255,0.03); }

.log-icon-wrap {
  width: 36px; height: 36px; flex-shrink: 0; border: 1px solid;
  display: flex; align-items: center; justify-content: center; margin-top: 2px;
}
.log-icon-wrap svg { width: 15px; height: 15px; }
.entry-in  .log-icon-wrap { border-color: var(--gold-mid);    color: var(--gold);  background: var(--gold-dim); }
.entry-out .log-icon-wrap { border-color: var(--blue-border); color: var(--blue);  background: var(--blue-dim); }

.log-body { flex: 1; min-width: 0; }
.log-main { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; flex-wrap: wrap; }
.log-type { font-size: 13px; font-weight: 600; color: var(--cream); }
...
.log-badge svg { width: 9px; height: 9px; }
.log-source-badge { color: var(--cream-2); border-color: rgba(189,181,172,0.30); background: rgba(189,181,172,0.08); }
.log-city-badge   { color: var(--green);   border-color: var(--green-border);    background: var(--green-dim); }

.log-meta        { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; margin-bottom: 6px; }
.log-date        { font-size: 11px; color: var(--cream-2); }
.log-sep         { color: var(--cream-3); font-size: 11px; }
.log-time-inline { font-family: var(--font-mono); font-size: 11px; color: var(--cream-2); }
...
.log-reason span {
  font-size: 11px; color: var(--cream-2);
...
.ltr-time { font-family: var(--font-mono); font-size: 14px; font-weight: 500; color: var(--cream); }
.ltr-type {
  font-size: 9px; font-weight: 700; letter-spacing: .10em; padding: 1px 6px; border: 1px solid;
}
.ltr-in  { color: var(--gold);  border-color: var(--gold-mid);    background: var(--gold-dim); }
.ltr-out { color: var(--blue);  border-color: var(--blue-border); background: var(--blue-dim); }
...
.logs-empty svg  { width: 44px; height: 44px; color: var(--cream-3); }
.logs-empty p    { margin: 0; font-size: 14px; font-weight: 500; color: var(--cream); }
.logs-empty span { font-size: 12px; color: var(--cream-2); }
...
.page-btn {
  ...
  background: transparent; border: 1px solid rgba(189,181,172,0.28);
  color: var(--cream-2); cursor: pointer; transition: all .13s;
}
.page-btn svg { width: 14px; height: 14px; }
.page-btn:hover:not(:disabled) { border-color: var(--gold-mid); color: var(--gold); }
.page-btn:disabled { opacity: .3; cursor: not-allowed; }
.page-info { font-family: var(--font-mono); font-size: 12px; color: var(--cream-2); }
...
.modal-in  { border-color: rgba(201,169,110,0.45); }
.modal-out { border-color: rgba(239,107,107,0.40); }
...
.modal-panel::before { top: -1px; left: -1px; border-top: 2px solid; border-left: 2px solid; }
.modal-panel::after  { bottom: -1px; right: -1px; border-bottom: 2px solid; border-right: 2px solid; }
.modal-in::before,  .modal-in::after  { border-color: var(--gold); }
.modal-out::before, .modal-out::after { border-color: var(--red); }
...
.modal-header-icon {
  width: 42px; height: 42px; flex-shrink: 0; border: 1px solid;
  display: flex; align-items: center; justify-content: center;
}
.modal-header-icon svg { width: 18px; height: 18px; }
.mhi-in  { border-color: var(--gold-mid); color: var(--gold); background: var(--gold-dim); }
.mhi-out { border-color: var(--red-border); color: var(--red); background: var(--red-dim); }

.modal-title {
  margin: 0; font-family: var(--font-serif); font-size: 18px; font-weight: 500;
  color: var(--cream); letter-spacing: .02em; line-height: 1;
}
.modal-subtitle { margin: 4px 0 0; font-size: 11px; color: var(--cream-2); font-family: var(--font-mono); }

.modal-close {
  margin-left: auto; width: 32px; height: 32px; flex-shrink: 0;
  background: transparent; border: 1px solid rgba(189,181,172,0.22);
  color: var(--cream-3); cursor: pointer; display: flex; align-items: center; justify-content: center;
  transition: all .14s;
}
.modal-close:hover:not(:disabled) { border-color: var(--red-border); color: var(--red); }
.modal-close:disabled { opacity: .4; cursor: not-allowed; }
.modal-close svg { width: 14px; height: 14px; }

.modal-divider { height: 1px; background: var(--border); }

.modal-body { padding: 22px 24px; display: flex; flex-direction: column; gap: 12px; }
...
.reason-label-text {
  ...
  letter-spacing: .09em; color: var(--cream-2);
}
.reason-required {
  font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .10em;
  padding: 2px 7px; border: 1px solid var(--red-border);
  color: var(--red); background: var(--red-dim);
}

/* Quick presets */
.reason-presets { display: flex; gap: 7px; flex-wrap: wrap; }
.preset-chip {
  padding: 4px 11px; border: 1px solid rgba(189,181,172,0.25);
  background: rgba(189,181,172,0.06); color: var(--cream-2);
  font-family: var(--font-body); font-size: 11px; font-weight: 500;
  cursor: pointer; transition: all .15s; white-space: nowrap;
}
.preset-chip:hover { border-color: var(--gold-mid); color: var(--gold); background: var(--gold-dim); }
.preset-active { border-color: var(--gold-mid) !important; color: var(--gold) !important; background: var(--gold-dim) !important; }

/* Textarea wrapper */
.reason-input-wrap {
  display: flex; align-items: flex-start; gap: 10px;
  border: 1px solid rgba(189,181,172,0.28);
  background: rgba(255,255,255,0.03);
  padding: 10px 12px;
  transition: border-color .2s, box-shadow .2s;
}
.input-focus { border-color: var(--gold-mid); box-shadow: 0 0 0 3px rgba(201,169,110,0.10); }
.input-error { border-color: var(--red-border); box-shadow: 0 0 0 3px rgba(239,107,107,0.10); }

.reason-input-icon {
  width: 14px; height: 14px; flex-shrink: 0; color: var(--cream-3); margin-top: 2px;
}
.reason-textarea {
...
.reason-textarea::placeholder { color: var(--cream-3); }

.reason-footer-row {
  display: flex; align-items: center; justify-content: space-between; gap: 12px; min-height: 18px;
}
.reason-hint { font-size: 11px; color: var(--cream-3); }
.reason-error-msg {
  display: inline-flex; align-items: center; gap: 5px;
  font-size: 11px; color: var(--red); font-weight: 500;
}
.reason-error-msg svg { width: 12px; height: 12px; flex-shrink: 0; }
.reason-char-count { font-family: var(--font-mono); font-size: 10px; color: var(--cream-3); flex-shrink: 0; }
...
.modal-btn-cancel:hover:not(:disabled) { border-color: rgba(189,181,172,0.50); color: var(--cream); }
.modal-btn-cancel:disabled { opacity: .4; cursor: not-allowed; }
...
.modal-btn-confirm svg { width: 14px; height: 14px; flex-shrink: 0; }
.confirm-in {
  background: var(--gold-dim); border-color: var(--gold-mid); color: var(--gold);
}
...
.confirm-out {
  background: var(--red-dim); border-color: var(--red-border); color: var(--red);
}
.confirm-out:hover:not(:disabled) {
  background: rgba(239,107,107,0.26); border-color: var(--red);
  box-shadow: 0 0 24px rgba(239,107,107,0.20);
}
.modal-btn-confirm:disabled { opacity: .4; cursor: not-allowed; }

/* ── Modal transitions ── */
.modal-enter-active { transition: all .25s cubic-bezier(0.34,1.56,0.64,1); }
.modal-leave-active { transition: all .18s ease; }
.modal-enter-from   { opacity: 0; transform: scale(0.94) translateY(12px); }
.modal-leave-to     { opacity: 0; transform: scale(0.97) translateY(6px); }
...
.toast-notif {
  ...
  font-family: var(--font-body); min-width: 280px; max-width: 420px;
  box-shadow: 0 16px 48px rgba(0,0,0,0.60);
}
.toast-notif svg { width: 16px; height: 16px; flex-shrink: 0; }
.toast-success { background: #08140E; border: 1px solid var(--green-border); color: var(--green); }
.toast-error   { background: #140808; border: 1px solid var(--red-border);   color: var(--red); }
.toast-enter-active { transition: all .28s cubic-bezier(0.34,1.56,0.64,1); }
...
@media (max-width: 768px) {
  .attendance-page    { padding: 20px 16px; }
  .clock-card         { padding: 24px 20px; gap: 28px; }
  .stats-strip        { flex-direction: column; }
  .stat-tile-divider  { width: auto; height: 1px; }
  .stat-tile          { padding: 16px 20px; }
  .log-time-right     { display: none; }
  .logs-section       { padding: 20px 16px; }
  .meta-strip         { gap: 8px; }
  .today-summary-bar  { padding: 12px 16px; }
  .modal-panel        { max-width: 100%; }
  .modal-header       { padding: 18px 18px; }
  .modal-body         { padding: 18px 18px; }
  .modal-footer       { padding: 14px 18px; }
  .reason-presets     { gap: 6px; }
}
</style>