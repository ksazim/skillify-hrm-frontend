    ...
    <div class="login-left">
      ...
      <div class="left-brand">
        ...
        <div class="brand-text">
          <span class="brand-name">{{ systemData?.brand_name || 'Nexara' }}</span>
      ...
      <div class="left-quote">
        ...
        <p class="quote-text">Precision in people,<br>excellence in operations.</p>
            ...
            <Transition name="btn-text" mode="out-in">
              <span v-if="!loading" key="idle" class="btn-idle">
                ...
                <svg viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M3 10a1 1 0 011-1h10.586l-2.293-2.293a1 1 0 111.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H4a1 1 0 01-1-1z" clip-rule="evenodd"/>
                </svg>
              </span>
...
function validateEmail() {
  if (!form.email) { fieldErrors.email = 'Email is required'; return false }
...
const fetchSystemSettings = async () => {
  try {
    ...
    if (response.data?.system) systemData.value = response.data.system
  } catch (error) {
    console.error('Error fetching system settings:', error)
  }
}

const getLogoUrl = (path) => path ? `${process.env.VUE_APP_BASE_URL}/${path}` : ''
...
.admin-login {
  ...
  --gold-glow-str:  rgba(201,169,110,0.20);
  --onyx:           #0D0D0F;
  --onyx-2:         #131316;
  --onyx-3:         #1A1A1F;
  --onyx-4:         #222228;
...
.left-glow--top    { top:-120px;  left:-80px;  width:360px; height:360px; background:radial-gradient(circle,rgba(201,169,110,0.10) 0%,transparent 65%); }
.left-glow--mid    { top:40%;     left:30%;    width:280px; height:280px; background:radial-gradient(circle,rgba(201,169,110,0.05) 0%,transparent 70%); }
.left-glow--bottom { bottom:-80px;right:-60px; width:300px; height:300px; background:radial-gradient(circle,rgba(201,169,110,0.07) 0%,transparent 65%); }

/* Grid lines */
.grid-lines { position:absolute; inset:0; pointer-events:none; }
.grid-line   { position:absolute; }
.grid-line--v { top:0; bottom:0; width:1px; background:linear-gradient(to bottom,transparent,rgba(201,169,110,0.06) 30%,rgba(201,169,110,0.06) 70%,transparent); }
.grid-line--h { left:0; right:0; height:1px; background:linear-gradient(to right,transparent,rgba(201,169,110,0.06) 30%,rgba(201,169,110,0.06) 70%,transparent); }
...
.brand-name { font-family:'Cormorant Garamond','Georgia',serif; font-size:22px; font-weight:500; letter-spacing:.08em; color:var(--text-primary); line-height:1; }
.brand-sub  { font-size:9px; font-weight:300; letter-spacing:.22em; text-transform:uppercase; color:var(--gold); line-height:1; }
...
.stat-value { font-family:'Cormorant Garamond','Georgia',serif; font-size:18px; font-weight:500; color:var(--gold-light); letter-spacing:.04em; }
.stat-label { font-size:9px; text-transform:uppercase; letter-spacing:.18em; color:var(--text-muted); }
...
.logo-mark-sm::before { content:''; position:absolute; inset:4px; border:1px solid var(--gold); opacity:.3; }
...
.toggle-pw:disabled { pointer-events:none; }

/* Error states */
.field-group--error .field-wrap input { border-color: rgba(201,60,60,0.5); }
.field-error { display:block; font-size:11px; color:#F09595; margin-top:5px; letter-spacing:.02em; }
...
.btn-idle svg { width:16px; height:16px; }
.btn-busy { gap: 10px; }
...
/* ── Transitions ── */
.alert-enter-active { transition: opacity .2s ease, transform .2s ease; }
.alert-leave-active { transition: opacity .15s ease; }
.alert-enter-from   { opacity:0; transform:translateY(-6px); }
.alert-leave-to     { opacity:0; }

.btn-text-enter-active { transition: opacity .15s ease, transform .15s ease; }
...
@media (max-width: 860px) {
  ...
  .login-right { background: var(--onyx-2); }
}
