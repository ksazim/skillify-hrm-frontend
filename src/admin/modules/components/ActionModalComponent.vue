  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="modelValue" class="modal-overlay" @click.self="handleOverlayClick">
        <div
          class="modal-box"
          :class="[`modal-box--${size}`, `modal-box--${mode}`]"
          role="dialog"
            ...
            <div class="modal-header__inner">
              <div class="modal-header__title-wrap">
                ...
                <div class="modal-mode-mark" :class="`mode-mark--${mode}`">
                  <!-- create -->
                  <svg v-if="mode === 'create'" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd"/></svg>
                  <!-- edit -->
                  <svg v-else-if="mode === 'edit'" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                  <!-- delete -->
                  <svg v-else-if="mode === 'delete'" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                  <!-- view -->
          ...
          <div class="modal-body">
            ...
            <template v-if="mode === 'delete'">
              <div class="delete-zone">
                ...
                <p class="delete-zone__message">{{ deleteMessage || `Are you sure you want to delete this ${entityName}?` }}</p>
                ...
                >
                  ...
                  <div class="view-row__value">
                    ...
                    <template v-if="field.type === 'file' && initialData[field.name]">
                      ...
                      <a v-else :href="getExistingImageUrl(field)" target="_blank" rel="noopener noreferrer" class="view-file-link">
                    ...
                    <template v-else-if="Array.isArray(initialData[field.name])">
                      ...
                      <div v-else class="view-tags">
                        <span v-for="item in initialData[field.name]" :key="item" class="view-tag">{{ formatArrayItem(item, field) }}</span>
                      </div>
                    </template>
                    ...
                    <template v-else-if="typeof initialData[field.name] === 'boolean'">
                      <span class="bool-badge" :class="initialData[field.name] ? 'bool-badge--yes' : 'bool-badge--no'">
                        <svg v-if="initialData[field.name]" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                ...
                >
                  ...
                  <div v-if="['text','email','password','url'].includes(field.type)" class="form-input-wrap">
                    ...
                    <button
                      ...
                      type="button"
                      class="form-pw-toggle"
                      @click="togglePassword(field.name)"
                      :disabled="loading"
                    >
                      <svg v-if="!showPassword[field.name]" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"/></svg>
                  ...
                  <div v-else-if="field.type === 'select'" class="form-select-wrap">
                    ...
                    <svg class="form-select-arrow" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                  </div>
                  ...
                  <div v-else-if="field.type === 'date'" class="form-input-wrap">
                    <svg class="form-input-icon" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd"/></svg>
                    <input type="date" :id="field.name" class="form-input form-input--icon" :required="field.required" :disabled="field.disabled||loading" :readonly="field.readonly" :min="field.min" :max="field.max" v-model="formData[field.name]" @blur="validateField(field)"/>
                  </div>

                  <!-- DateTime -->
                  <input v-else-if="field.type === 'datetime'" type="datetime-local" :id="field.name" class="form-input" :required="field.required" :disabled="field.disabled||loading" :readonly="field.readonly" v-model="formData[field.name]" @blur="validateField(field)"/>

                  <!-- Time -->
                  <input v-else-if="field.type === 'time'" type="time" :id="field.name" class="form-input" :required="field.required" :disabled="field.disabled||loading" :readonly="field.readonly" v-model="formData[field.name]" @blur="validateField(field)"/>

                  <!-- Checkbox -->
                  <div v-else-if="field.type === 'checkbox'" class="form-checkbox-wrap">
                    <label :for="field.name" class="form-checkbox-label">
                      <input type="checkbox" :id="field.name" class="form-checkbox-input" :disabled="field.disabled||loading" v-model="formData[field.name]"/>
                  ...
                  <div v-else-if="field.type === 'radio'" class="form-radio-group">
                    <label v-for="opt in field.options" :key="opt.value" class="form-radio-label">
                      <input type="radio" class="form-radio-input" :name="field.name" :value="opt.value" :disabled="field.disabled||loading" v-model="formData[field.name]"/>
                  ...
                  <div v-else-if="field.type === 'file'" class="form-file-wrap">
                    ...
                    <div v-if="field.accept?.includes('image')" class="form-file-preview">
                      <div class="form-file-preview__frame">
                        ...
                        <button
                          v-if="filePreview[field.name]"
                          type="button"
                          class="form-file-preview__remove"
                      ...
                      <p class="form-file-preview__label">{{ filePreview[field.name] ? (isFileObject(formData[field.name]) ? 'New image selected' : 'Current image') : 'No image selected' }}</p>
                    </div>
                    ...
                    <div class="form-file-input-wrap">
                      ...
                      <input type="file" :id="field.name" class="form-file-hidden" :accept="field.accept" :multiple="field.multiple" :disabled="field.disabled||loading" @change="handleFileChange(field, $event)"/>
                    </div>
                  </div>

                  <!-- Color -->
                  <div v-else-if="field.type === 'color'" class="form-color-wrap">
                    <input type="color" :id="field.name" class="form-color-input" :disabled="field.disabled||loading" v-model="formData[field.name]"/>
                    <span class="form-color-value">{{ formData[field.name] }}</span>
                  </div>

                  <!-- Range -->
                  <div v-else-if="field.type === 'range'" class="form-range-wrap">
                    <input type="range" :id="field.name" class="form-range" :min="field.min||0" :max="field.max||100" :step="field.step||1" :disabled="field.disabled||loading" v-model.number="formData[field.name]"/>
                    <span class="form-range-value">{{ formData[field.name] }}</span>
                  </div>

                  <!-- Custom slot -->
                  <slot v-else-if="field.type === 'custom'" :name="`field-${field.name}`" :field="field" :value="formData[field.name]" :updateValue="(val) => formData[field.name] = val"/>

                  <!-- Description -->
                  <small v-if="field.description && !field.hidden" class="form-description">{{ field.description }}</small>
...
.modal-box--large  { max-width: 820px; }
...
.rule--delete { background: linear-gradient(90deg, transparent, rgba(240,149,149,0.5), rgba(240,149,149,0.9), rgba(240,149,149,0.5), transparent); }
.rule--view   { background: linear-gradient(90deg, transparent, rgba(133,183,235,0.5), rgba(133,183,235,0.9), rgba(133,183,235,0.5), transparent); }
...
.modal-header__title-wrap { display:flex; align-items:center; gap:14px; min-width:0; }
...
.modal-mode-mark::before { content:''; position:absolute; inset:3px; border:1px solid var(--gold); opacity:.2; }
.modal-mode-mark svg { width:15px; height:15px; }
.mode-mark--delete { color:#F09595; border-color:rgba(240,149,149,0.3); }
.mode-mark--delete::before { border-color:rgba(240,149,149,0.4); }
.mode-mark--view   { color:#85B7EB; border-color:rgba(133,183,235,0.3); }
.mode-mark--view::before   { border-color:rgba(133,183,235,0.4); }

.modal-mode-label {
  font-size: 8.5px; text-transform:uppercase; letter-spacing:.18em;
  color: var(--text-muted); margin: 0 0 2px; line-height:1;
}
.modal-title {
...
.modal-close:hover:not(:disabled) { color:var(--text-primary); border-color:var(--border-strong); background:var(--smoke); }
...
.delete-zone {
  ...
  text-align:center; gap:12px; padding:8px 0;
}
.delete-zone__mark {
  width:56px; height:56px;
  border:1px solid rgba(240,149,149,0.35);
  display:flex; align-items:center; justify-content:center;
  position:relative; color:#F09595;
}
.delete-zone__mark::before { content:''; position:absolute; inset:5px; border:1px solid rgba(240,149,149,0.4); opacity:.4; }
.delete-zone__mark svg { width:22px; height:22px; }
.delete-zone__message { font-size:14px; color:var(--text-primary); letter-spacing:.02em; margin:0; }
.delete-zone__warning  { font-size:12px; color:#F09595; letter-spacing:.04em; margin:0; }

.entity-detail-grid { width:100%; border:1px solid var(--border); background:var(--onyx-3); margin-top:8px; }
.entity-detail-row  { display:flex; padding:10px 14px; border-bottom:1px solid var(--border); gap:12px; }
.entity-detail-row:last-child { border-bottom:none; }
.entity-detail-key { font-size:10px; text-transform:uppercase; letter-spacing:.14em; color:var(--text-muted); min-width:100px; flex-shrink:0; padding-top:1px; }
.entity-detail-val { font-size:13px; color:var(--text-secondary); word-break:break-word; }
...
.view-row__label { font-size:9px; text-transform:uppercase; letter-spacing:.16em; color:var(--text-muted); padding-top:2px; }
.view-row__value { font-size:13px; color:var(--text-secondary); word-break:break-word; line-height:1.5; }
.view-empty { color:var(--text-muted); font-size:12px; }

.view-tags { display:flex; flex-wrap:wrap; gap:6px; }
.view-tag {
  display:inline-block; padding:3px 10px; font-size:10.5px;
  letter-spacing:.06em; text-transform:uppercase;
  background:var(--gold-glow); border:1px solid var(--border-strong); color:var(--gold-light);
}

.bool-badge {
  display:inline-flex; align-items:center; gap:5px;
  padding:3px 10px; font-size:10.5px; letter-spacing:.08em;
  text-transform:uppercase; border:1px solid;
}
.bool-badge svg { width:11px; height:11px; }
.bool-badge--yes { background:rgba(29,158,117,0.09); border-color:rgba(29,158,117,0.35); color:#5DCAA5; }
.bool-badge--no  { background:rgba(201,60,60,0.08);  border-color:rgba(201,60,60,0.3);  color:#F09595; }

.view-img-wrap { max-width:100%; }
.view-img { max-width:100%; max-height:320px; object-fit:contain; border:1px solid var(--border); display:block; }
.view-file-link { display:inline-flex; align-items:center; gap:7px; font-size:12.5px; color:var(--gold); text-decoration:none; transition:opacity .14s; }
.view-file-link:hover { opacity:.8; }
.view-file-link svg { width:14px; height:14px; }
...
.form-alert {
  ...
  padding:11px 14px; border-left:2px solid;
  font-size:13px; letter-spacing:.02em;
}
.form-alert svg { width:15px; height:15px; flex-shrink:0; margin-top:1px; }
.form-alert--error { background:rgba(201,60,60,0.07); border-color:rgba(201,60,60,0.5); color:#F09595; }
...
.form-required { color:#F09595; }
.form-description { font-size:11px; color:var(--text-muted); letter-spacing:.03em; }
...
.form-input[readonly],
.form-textarea[readonly] { opacity:.6; }

.form-input-wrap { position:relative; display:flex; align-items:center; }
.form-input--icon { padding-left:36px; }
.form-input-icon { position:absolute; left:11px; width:13px; height:13px; color:var(--gold-dim); pointer-events:none; }

.form-pw-toggle { position:absolute; right:10px; width:26px; height:26px; background:none; border:none; cursor:pointer; color:var(--text-muted); display:flex; align-items:center; justify-content:center; padding:0; transition:color .14s; }
.form-pw-toggle:hover { color:var(--gold); }
.form-pw-toggle svg { width:14px; height:14px; }

.form-textarea { resize:vertical; min-height:96px; }

.form-select-wrap { position:relative; }
.form-select { padding-right:32px; cursor:pointer; }
.form-select option { background:var(--onyx-4); color:var(--text-primary); }
.form-select-arrow { position:absolute; right:10px; top:50%; transform:translateY(-50%); width:13px; height:13px; color:var(--gold-dim); pointer-events:none; }

.form-multiselect { min-height:100px; padding:6px 10px; }
.form-multiselect option { padding:4px 6px; background:var(--onyx-4); color:var(--text-secondary); }
.form-multiselect option:checked { background:var(--gold-glow); color:var(--gold-light); }

/* Checkbox */
.form-checkbox-input { display:none; }
.form-checkbox-wrap  { padding:4px 0; }
...
.form-checkbox-text  { font-size:13px; color:var(--text-secondary); letter-spacing:.02em; }

/* Radio */
.form-radio-group { display:flex; flex-direction:column; gap:8px; padding-top:2px; }
.form-radio-input { display:none; }
.form-radio-label { display:inline-flex; align-items:center; gap:9px; cursor:pointer; user-select:none; }
.form-radio-dot   { width:14px; height:14px; border-radius:50%; border:1px solid var(--border-strong); background:var(--onyx-4); flex-shrink:0; position:relative; transition:border-color .14s; }
.form-radio-input:checked + .form-radio-dot { border-color:var(--gold); background:var(--gold-glow); }
.form-radio-input:checked + .form-radio-dot::after { content:''; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:5px; height:5px; border-radius:50%; background:var(--gold); }
.form-radio-text  { font-size:13px; color:var(--text-secondary); letter-spacing:.02em; }

/* File upload */
.form-file-wrap { display:flex; flex-direction:column; gap:14px; }
.form-file-preview { display:flex; flex-direction:column; align-items:center; gap:8px; }
.form-file-preview__frame { width:160px; height:160px; border:1px dashed var(--border-strong); background:var(--onyx-4); position:relative; display:flex; align-items:center; justify-content:center; overflow:hidden; }
.form-file-preview__img   { width:100%; height:100%; object-fit:cover; display:block; }
.form-file-preview__empty { display:flex; flex-direction:column; align-items:center; gap:6px; color:var(--text-muted); }
.form-file-preview__empty svg { width:24px; height:24px; }
.form-file-preview__empty span { font-size:11px; letter-spacing:.08em; }
.form-file-preview__remove {
  position:absolute; top:6px; right:6px;
  width:24px; height:24px;
  background:rgba(240,149,149,0.85); border:none; cursor:pointer;
  display:flex; align-items:center; justify-content:center;
  color:#fff; opacity:0; transition:opacity .14s;
  padding:0;
}
.form-file-preview__frame:hover .form-file-preview__remove { opacity:1; }
.form-file-preview__remove svg { width:11px; height:11px; }
.form-file-preview__label { font-size:11px; color:var(--text-muted); letter-spacing:.06em; margin:0; }
...
.form-file-btn:hover:not(.form-file-btn--disabled) { border-color:var(--gold); color:var(--gold); background:var(--gold-glow); }
.form-file-btn--disabled { opacity:.4; cursor:not-allowed; }
.form-file-btn svg { width:13px; height:13px; flex-shrink:0; }

/* Color */
.form-color-wrap { display:flex; align-items:center; gap:12px; }
.form-color-input { width:40px; height:36px; border:1px solid var(--border); background:none; cursor:pointer; padding:2px; }
.form-color-value  { font-size:12px; font-family:'Courier New',monospace; color:var(--gold); letter-spacing:.04em; }

/* Range */
.form-range-wrap { display:flex; align-items:center; gap:12px; }
.form-range { flex:1; -webkit-appearance:none; height:2px; background:var(--border-strong); outline:none; cursor:pointer; }
.form-range::-webkit-slider-thumb { -webkit-appearance:none; width:14px; height:14px; background:var(--gold); border:1px solid var(--gold-dim); cursor:pointer; transition:box-shadow .14s; }
.form-range::-webkit-slider-thumb:hover { box-shadow:0 0 0 4px rgba(201,169,110,0.15); }
.form-range::-moz-range-thumb { width:14px; height:14px; background:var(--gold); border:1px solid var(--gold-dim); border-radius:0; cursor:pointer; }
.form-range-value { font-family:'Cormorant Garamond','Georgia',serif; font-size:16px; font-weight:500; color:var(--gold-light); min-width:36px; text-align:right; }

/* Field error */
.form-field-error { display:inline-flex; align-items:center; gap:5px; font-size:11px; color:#F09595; letter-spacing:.02em; }
.form-field-error svg { width:12px; height:12px; flex-shrink:0; }
...
.modal-btn {
  ...
  padding:10px 22px; min-width:100px;
...
.modal-btn--cancel:hover:not(:disabled) { border-color:var(--border-strong); color:var(--text-secondary); background:var(--smoke); }

/* Primary (create/edit) — gold gradient */
.modal-btn--primary {
  background:linear-gradient(135deg, var(--gold-dim), var(--gold));
  border-color:transparent; color:var(--onyx);
}
.modal-btn--primary::before { content:''; position:absolute; inset:0; background:rgba(255,255,255,0.08); opacity:0; transition:opacity .14s; }
.modal-btn--primary:hover:not(:disabled)::before { opacity:1; }
.modal-btn--primary:hover:not(:disabled) { box-shadow:0 4px 18px rgba(201,169,110,0.28); transform:translateY(-1px); }

/* Danger (delete) */
.modal-btn--danger { background:rgba(240,149,149,0.10); border-color:rgba(240,149,149,0.4); color:#F09595; }
.modal-btn--danger:hover:not(:disabled) { background:rgba(240,149,149,0.18); border-color:#F09595; }

/* Ghost (view close) */
.modal-btn--ghost { background:transparent; border-color:var(--border-strong); color:var(--text-secondary); }
.modal-btn--ghost:hover:not(:disabled) { border-color:var(--gold-dim); color:var(--gold-light); background:var(--gold-glow); }

/* Spinner */
.modal-btn__busy { display:flex; align-items:center; gap:8px; }
.modal-spinner { width:14px; height:14px; animation:spin .8s linear infinite; }
...
input[type="time"]::-webkit-calendar-picker-indicator {
  filter:invert(.55) sepia(1) saturate(2) hue-rotate(5deg);
...
.modal-enter-active .modal-box { transition:transform .22s cubic-bezier(.22,.68,0,1.2); }
.modal-leave-active .modal-box  { transition:transform .16s ease; }
.modal-enter-from .modal-box { transform:scale(.96) translateY(8px); }
.modal-leave-to   .modal-box { transform:scale(.97); }

.alert-enter-active { transition:max-height .2s ease, opacity .18s ease; max-height:80px; overflow:hidden; }
.alert-leave-active { transition:max-height .14s ease, opacity .12s ease; }
.alert-enter-from, .alert-leave-to { max-height:0; opacity:0; }

.btn-text-enter-active { transition:opacity .14s ease, transform .14s ease; }
.btn-text-leave-active { transition:opacity .1s ease; }
.btn-text-enter-from   { opacity:0; transform:translateY(4px); }
...
@media (max-width:640px) {
  ...
  .view-row       { grid-template-columns:1fr; gap:4px; }
  .view-row__label { padding-top:0; }
}
</style>