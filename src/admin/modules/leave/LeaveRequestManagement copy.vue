<template>
  ...
  >
    ...
    >
      ...
      <template #cell-status="{ row }">
        <span class="lr-status-badge" :class="'lr-status-' + row.status">
          {{ row.status }}
        </span>
      </template>
      ...
      <div class="lr-modal-shell">
        ...
        <div class="lr-modal-body">
          ...
          <div class="lr-form-grid">
            ...
            <!-- Leave Policy -->
            <!-- FIX 1: Policy dropdown is shown in both create and edit modes.
                 In edit mode it's disabled (policy cannot be changed) but the
                 selected value is still rendered correctly because leavePolicies
                 is loaded on mount and form.leave_policy_id is set from the record. -->
            <div class="lr-form-field lr-field-full">
              <label class="lr-form-label">Leave Policy <span class="lr-req">*</span></label>

              <!-- Loading skeleton while policies are being fetched -->
              ...
              <template v-else>
                ...
                <!-- In edit mode, also show the policy name as read-only text so
                     it's visible even when the <select> is disabled/greyed out -->
          ...
          <div class="lr-form-grid">
            <div class="lr-form-field lr-field-full">
              ...
              <!-- Existing file hint in edit mode -->
      ...
      <div class="lr-modal-shell lr-action-shell">
        ...
        <div class="lr-modal-head">
          <div class="lr-modal-head-left">
            <div
              class="lr-modal-icon"
              :class="actionModalMode === 'approve' ? 'lr-icon-approve' : 'lr-icon-reject'"
            >
            ...
            <div>
              ...
              <p class="lr-modal-subtitle">
                {{ actionModalMode === 'approve'
                  ? 'Confirm approval — optionally add a note for the employee'
                  : 'Provide a reason so the employee can reapply if needed' }}
              </p>
            </div>
          </div>
          ...
          <div class="lr-action-summary">
            ...
            <div class="lr-action-summary-row">
              ...
              <span class="lr-action-summary-val">
                ...
                <em class="lr-action-days">
                  {{ actionItem?.total_days }} day{{ actionItem?.total_days != 1 ? 's' : '' }}
                </em>
              </span>
            </div>
  ...
  <!-- ══════════════════════════════════════════════════
       VIEW MODAL  (FIX 3 — custom modal, not BaseModal)
       Shows all request details + an inline file preview
       / download for PDF, DOC, image attachments.
  ══════════════════════════════════════════════════ -->
  <Teleport to="body">
      ...
      <div class="lr-modal-shell lr-view-shell">
        ...
        <div class="lr-modal-body">
          ...
          <div class="lr-view-grid">
            ...
            <div class="lr-view-row">
              <span class="lr-view-label">Status</span>
              <span>
                <span class="lr-status-badge" :class="'lr-status-' + selectedItem?.status">
                  {{ selectedItem?.status ?? '—' }}
                </span>
              </span>
            </div>
            ...
            <div v-if="selectedItem?.notes" class="lr-view-row lr-view-row--full">
              <span class="lr-view-label">Admin Notes</span>
              <span class="lr-view-val lr-view-reason" style="color:rgba(201,169,110,0.80)">{{ selectedItem.notes }}</span>
            </div>

          </div>

          <!-- ── FIX 3: Attachment section ── -->
          <template v-if="selectedItem?.attachment">
            ...
            <div class="lr-attachment-card">
              ...
              <!-- Inline image preview (for jpg/png/jpeg only) -->
              ...
              <div v-else-if="isPdf(selectedItem.attachment) && attachmentPreviewUrl" class="lr-attachment-pdf-wrap">
                <iframe
                  ...
                  class="lr-attachment-pdf"
                  title="PDF Preview"
                  frameborder="0"
                ></iframe>
              </div>

              <!-- Load preview button for supported types when not yet loaded -->
              <div
          ...
          <!-- Admin quick-action buttons inside view modal -->
          <div v-if="isAdminUser && selectedItem?.status === 'pending'" class="lr-view-admin-actions">
            <div class="lr-form-section-title" style="margin-top:28px">Admin Actions</div>
            <div class="lr-view-action-row">
              <button
                class="lr-view-action-btn lr-view-action-approve"
                @click="handleViewModalAction('approve')"
              >
              ...
              <button
                class="lr-view-action-btn lr-view-action-reject"
                @click="handleViewModalAction('reject')"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
                </svg>
        ...
        <div class="lr-modal-foot">
          ...
          <!-- Admin can jump straight to approve/reject from the view modal footer too -->
          <template v-if="isAdminUser && selectedItem?.status === 'pending'">
  ...
  <!-- ── Delete confirm modal (BaseModal) ── -->
  <BaseModal
...
// ── Current user (fetched from API — not stored in localStorage) ──────────────
// Holds the full user object from GET /user so we never touch localStorage
// for role checks. Populated once in onMounted before any other data loads.
const currentUser = ref(null)

const loadCurrentUser = async () => {
  try {
    const data = await apiFetch('/user')
    // API shape: { user: { id, name, roles: [{ name: 'Admin' }, ...] } }
    currentUser.value = data.user ?? data
  } catch (e) {
    console.error('Could not load current user:', e)
    // Leave currentUser null — isAdminUser will safely default to false.
  }
}

// Derives the primary role name from the Spatie roles array.
// Handles: roles: [{ name: 'Admin' }]  →  'admin'
...
const isAdminUser = computed(() =>
  ['admin', 'super_admin', 'superadmin', 'hr', 'manager'].includes(userRole.value)
)
...
const pageStats = computed(() => [
  ...
  { label: 'Pending',  value: items.value.filter(i => i.status === 'pending').length  },
  { label: 'Approved', value: items.value.filter(i => i.status === 'approved').length },
  { label: 'Rejected', value: items.value.filter(i => i.status === 'rejected').length },
])
...
// FIX 1: leavePolicies loaded once on mount and never cleared between opens.
const leavePolicies   = ref([])
const policiesLoading = ref(false)

// FIX 1: Compute the human-readable label of the currently selected policy.
// Used in edit mode so the disabled <select> value is clearly visible.
...
const actionModalMode  = ref('approve')   // 'approve' | 'reject'
const actionItem       = ref(null)
const actionNotes      = ref('')
const actionSaving     = ref(false)
const actionModalNotif = ref({ show: false, type: 'success', message: '' })

// ── Attachment preview (view modal) ──────────────────────────────────────────
const attachmentPreviewUrl  = ref(null)   // blob URL for inline preview
...
// FIX 2: openActionModal is now called from BOTH the table row actions AND
// the view modal admin buttons — guarded by isAdminUser in tableActions.
const openActionModal = (item, mode) => {
  if (item.status !== 'pending') {
    notify('warning', `Only pending requests can be ${mode}d.`, { autoClose: 3000 })
    return
  }
...
// Handler called from the view modal approve/reject buttons.
// Closes the view modal first, then opens the action modal.
const handleViewModalAction = (mode) => {
  const item = { ...selectedItem.value }
  showViewModal.value = false
  // Small delay so the view modal finishes closing before action modal opens.
...
const tableColumns = [
  ...
  { key: 'status',     label: 'Status',   type: 'custom',  sortable: true,  width: '110px', minWidth: '100px', align: 'center' },
  { key: 'actions',    label: 'Actions',  type: 'actions', sortable: false, width: '220px', minWidth: '220px', align: 'center' },
]

// FIX 2: tableActions is computed so it reacts to role changes.
// Approve / Reject buttons are ONLY shown to admin roles.
...
// Loads a blob URL for inline preview (images + PDFs).
...
// FIX 1: loadLeavePolicies is called once on mount so policies are always
// available when the create or edit modal opens. It is NOT called again on
// every modal open, avoiding the race condition where the dropdown was empty
...
const openEditModal = async (item) => {
  if (item.status !== 'pending') {
    notify('warning', 'Only pending requests can be edited.', { autoClose: 3000 })
    return
  }
  try {
    ...
    editItem.value   = r
    form.value = {
      // FIX 1: Coerce to Number so it matches the numeric option :value bindings.
...
// FIX 3: openViewModal fetches fresh data and resets the preview blob URL.
const openViewModal = async (item) => {
  // Reset preview state so a stale preview from the previous view doesn't show.
  ...
  try {
    ...
    // Auto-load preview for images (small files load fast enough).
...
const handleAction = ({ action, row }) => {
  const map = {
    ...
    // FIX 2: approve / reject are wired here but only present in tableActions
    // when isAdminUser is true, so employees never see these buttons.
...
onMounted(async () => {
  // Load the current user first so role-gated UI (approve/reject buttons)
  // is resolved before the table renders. The other two can run in parallel.
...
/* ── Table fix — force fixed layout so columns respect declared widths ── */
...
:deep(.btn-approve) {
  ...
  border-color: rgba(110,207,169,0.40) !important;
...
/* ── Inline download button (edit modal existing file) ── */
.lr-inline-dl-btn {
...
/* Policies loading inline */
.lr-policies-loading {
  display: flex; align-items: center; gap: 8px;
  padding: 10px 13px; border: 1px solid rgba(201,169,110,0.14);
...
.lr-action-shell { max-width: 560px; }
.lr-view-shell   { max-width: 720px; }
...
.lr-icon-approve {
  background: rgba(110,207,169,0.10) !important; border-color: rgba(110,207,169,0.35) !important; color: #6ECFA9 !important;
}
.lr-icon-reject {
  background: rgba(239,107,107,0.10) !important; border-color: rgba(239,107,107,0.35) !important; color: #EF6B6B !important;
}
.lr-icon-view {
  background: rgba(133,183,235,0.10) !important; border-color: rgba(133,183,235,0.28) !important; color: rgba(133,183,235,0.85) !important;
}
...
.lr-modal-close {
  ...
  color: rgba(240,234,224,0.40); cursor: pointer; display: flex; align-items: center; justify-content: center;
...
/* ── Action modal summary card ── */
.lr-action-summary {
  background: rgba(255,255,255,0.03); border: 1px solid rgba(201,169,110,0.15); overflow: hidden;
}
...
.lr-action-summary-val {
  font-size: 12px; color: rgba(240,234,224,0.72); line-height: 1.5;
...
.lr-action-dl-btn {
  ...
  background: rgba(201,169,110,0.10); border: 1px solid rgba(201,169,110,0.35);
  color: #C9A96E; cursor: pointer; transition: all .14s; max-width: 100%;
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.lr-action-dl-btn:hover { background: rgba(201,169,110,0.20); border-color: rgba(201,169,110,0.55); }
...
/* ── Attachment card (view modal) ── */
.lr-attachment-card {
  border: 1px solid rgba(201,169,110,0.20); background: rgba(201,169,110,0.03); overflow: hidden;
}
.lr-attachment-info {
...
.lr-attachment-img {
  ...
  background: rgba(0,0,0,0.40); display: block;
}
...
.lr-no-attachment {
  ...
  padding: 18px; border: 1px dashed rgba(240,234,224,0.10);
  color: rgba(240,234,224,0.22); font-size: 12px;
}

/* Admin quick-actions inside view modal */
...
.lr-view-action-approve {
  background: rgba(110,207,169,0.10); border-color: rgba(110,207,169,0.40); color: #6ECFA9;
}
.lr-view-action-approve:hover { background: rgba(110,207,169,0.20); }
.lr-view-action-reject {
  background: rgba(239,107,107,0.10); border-color: rgba(239,107,107,0.40); color: #EF6B6B;
}
.lr-view-action-reject:hover { background: rgba(239,107,107,0.20); }

/* ══════════════════════════════════════════════════
   FORM FIELDS (shared between modals)
══════════════════════════════════════════════════ */
