...
<script setup>
import { ref, computed, defineProps, reactive, watch, nextTick } from 'vue'
...
// ── Menu definitions ────────────────────────────────────────────────────────────
// menu.perms  : at least one of these keywords must appear in a user permission
//               string for the entire menu to show. null = always show.
// sub.perm    : the exact permission string the user must have for this submenu
//               item to appear. null = always show inside an accessible menu.
const menuItems = [
  {
    section: 'Core',
    id: 'dashboard',
  ...
  },
  {
    ...
    submenu: [
      ...
      // { label: 'Leave policies',          link: '/admin/leave/policies',              perm: 'view leave policy' },
      // { label: 'Leave balances',          link: '/admin/leave/balances',              perm: 'view leave balance' },
      // { label: 'Approval hierarchy',      link: '/admin/leave/approval-hierarchy',    perm: 'view leave approval hierarchy' },
    ],
  },
  {
    section: 'Finance',
  ...
  },
  {
    ...
    submenu: [
      ...
      // { label: 'Tasks',           link: '/admin/project/tasks',      perm: 'view project task' },
      // { label: 'Team assignment', link: '/admin/project/teams',      perm: 'view project contributor' },
      // { label: 'Milestones',      link: '/admin/project/milestones', perm: 'view project' },
      // { label: 'Time tracking',   link: '/admin/project/time',       perm: 'view project task' },
    ],
  },
  {
...
// Two-level permission filtering:
//   1. Menu level  — show the menu if the user has at least one permission
//                    whose string contains any keyword in menu.perms.
//   2. Submenu level — show each sub-item if:
//                    a) sub.perm is null (always visible), OR
//                    b) the user has that exact permission string.
// If after filtering, a menu has zero visible sub-items, the menu itself
// is also hidden (no point showing an empty parent).
const visibleSections = computed(() => {
  ...
  menuItems.forEach(menu => {
    ...
    const filteredSubs = menu.submenu.filter(sub => {
      if (sub.perm === null) return true              // always visible
      return userPerms.includes(sub.perm)             // exact permission match
    })

    // Hide the whole menu if no sub-items passed the filter
